...
class UIFunctions(MainWindow):
    ...
    def switch_window(self,username,password):
        ...
        try:
            if validation:
                ...
                self.failed_login=QMessageBox()
                self.failed_login.setWindowTitle("Login Failed")
                self.failed_login.setIcon(QMessageBox.Warning)
                self.failed_login.setText("Login Failed")
                self.failed_login.setStandardButtons(QMessageBox.Ok)
                self.failed_login.exec_()
        except Exception as err:
            print(err)
    ...
    def update_msg_box(self):
        try:
            self.data,ok = QtWidgets.QInputDialog.getText(None,"Update Data","Enter Your New Password : ")
            for item in self.ui.table_data.selectedItems():
                id=item.text()
            if ok:
                db_connect.update_data(int(id),self.data)
        except Exception as err:
            self.error = QMessageBox()
            self.error.setWindowTitle("Warning")
            self.error.setText("You Must Click The ID !")
